...
package se.sics.gvod.network.serializers.netbootstrap;
 ...
 */
public class NetAddOverlayTest {
    ...
    @Test
    public void testResponse() throws Serializer.SerializerException, SerializationContext.MissingException, UnknownHostException {
        Address adr = new Address(Inet4Address.getLocalHost(), 10000, 123);
        Serializer<NetAddOverlay.Response> serializer = context.getSerializer(NetAddOverlay.Response.class);
        Assert.assertNotNull(serializer);
        AddOverlay.Response resp = new AddOverlay.Response(UUID.randomUUID(), ReqStatus.SUCCESS, 1);
        NetAddOverlay.Response expected = new NetAddOverlay.Response(new VodAddress(adr, -1), new VodAddress(adr, -1), UUID.randomUUID(), resp);
        int expectedSize = serializer.getSize(context, expected);
        ByteBuf buf = Unpooled.buffer();
        serializer.encode(context, buf, expected);
        ByteBuf newBuf = Unpooled.wrappedBuffer(buf.array());
        NetAddOverlay.Response decoded = serializer.decode(context, newBuf);
        decoded.rewriteDestination(adr);
        decoded.rewritePublicSource(adr);
        Assert.assertEquals(expected, decoded);
        Assert.assertEquals(expectedSize, buf.readableBytes());
